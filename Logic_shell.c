...
int main(int argc, char* argv[])
{
    ...
    if ((fd_file = open("Commands/Logic/Logic_Commands.txt", O_RDWR, 0664)) == -1)
    {
    ...
    while (1)
    {
        printf("LogicShell> "); // Display the prompt
        ...
        if (strcmp(args[0], "Highbit") == 0)
        {
            ...
            pid = CreateNewProcessAndExec("./Highbit", args); // Execute Highbit command
            free(args); // Free args memory
        }
        else if (strcmp(args[0], "DectoBin") == 0)
        {
            Document(args); // Log the command
            ...
            pid = CreateNewProcessAndExec("./DectoBin", args); // Execute DectoBin command
            free(args); // Free args memory
        }
        else if (strcmp(args[0], "DectoHex") == 0)
        {
            Document(args); // Log the command
            ...
            pid = CreateNewProcessAndExec("./DectoHex", args); // Execute DectoHex command
            free(args); // Free args memory
        }
        else if (strcmp(args[0], "History") == 0)
        {
            ...
            // Add space for "Logic" argument and NULL terminator
            temp = realloc(args, (arg_count + 2) * sizeof(char*));
            if (temp == NULL)
            {
            ...
            // Set "Logic" argument and NULL terminator
            args[1] = "Logic";
            args[2] = NULL;
